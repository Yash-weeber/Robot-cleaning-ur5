...
class LLMInterface:
    ...
    def render_prompt(self, iter_idx, feedback_text, bounds, guidance_text="",template_name=None):
    ...
    def call_ollama(self, prompt, token_limit=100000,model_name=None, use_critic=False):
        llm_settings = self.config['llm_settings']
        
        if model_name:
            target_model = model_name
        elif use_critic:
            target_model = llm_settings['critic_model'] # gemma3:27b
        else:
            target_model = llm_settings['llm_model']    # gpt-oss:120b
    ...
    def call_gemini(self, prompt, model_name=None, use_critic=False):
        llm_settings = self.config['llm_settings']
        
        # --- FIXED: Correctly defining the target_model variable ---
        if model_name:
            target_model = model_name
        elif use_critic:
            target_model = llm_settings['critic_model']
        else:
            target_model = llm_settings['llm_model']
