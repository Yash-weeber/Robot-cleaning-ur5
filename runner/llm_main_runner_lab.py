...
class LLM_Brain:
    ...
    def obtain_reward_from_file(self, reward_file_path):
        """Reads the reward from a file (for automated testing)"""
        try:
            with open(reward_file_path, 'r') as f:
                reward = float(f.read().strip())
                print(f"Read reward {reward} from file {reward_file_path}")
                return reward
        except Exception as e:
            print(f"Error reading reward from file: {e}. Defaulting to 0.")
            return 0.0
