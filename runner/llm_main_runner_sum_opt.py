...
from agent.llm_data_utils_sum_opt import (
...
def run_llm_optimization(config):
    ...
    inc_history = config['llm_settings'].get('include_history_in_prompt', False)
    ...
    for it in range(1 - n_warmup, max_iters + 1):
        ...
        if it < 0:
            ...
            feedback_text, guidance_text = build_llm_feedback(
                ...
                iter_log_data, traj_feedback_data, ee_traj_df, config, bounds, inc_reasoning=True
            )

            summary_prompt = llm.render_prompt(it + 1, feedback_text, bounds, guidance_text=guidance_text, summarize=True)

            feedback_text, guidance_text = build_llm_feedback(
                it + 1, pd.read_csv(config['logs']['weight_history_csv']),
                iter_log_data, traj_feedback_data, ee_traj_df, config, bounds, inc_reasoning=False
            )
            # try:
            # Use large token limit for coordinate tables
            if config['llm_settings']['llm_model'].startswith("gpt"):
                summary_text = llm.call_ollama(summary_prompt, token_limit=110000)
            elif config['llm_settings']['llm_model'].startswith("gemini"):
                summary_text = llm.call_gemini(summary_prompt)
            else:
            ...
            prompt = llm.render_prompt(it + 1, summary_text, bounds, guidance_text=guidance_text, summarize=False)
            ...
            save_dialog(config['logs']['dialog_dir'], it + 1, summary_prompt, summary_text, prompt, response)
            # except Exception as e:
