from shared_control.robot import RobotState
import numpy as np
import mujoco
...
from shared_control import Robot, Device
from shared_control.utils import ControllerConfig, Target
from shared_control.device import DeviceState
...
class OSC():
    ...
    """
    def __init__(self, robot: Robot, model: mujoco.MjModel, data: mujoco.MjData, input_device_configs: Tuple[str, Dict], nullspace_config : Dict = None, use_g=True, admittance=False):
        self.model = model
        self.data = data        
        self.robot = robot
    ...
    def generate(self, targets: Dict[str, Target]):
        ...
        Js, J_idxs = robot_state[RobotState.J.value]
        ...
        # M = self.robot.get_M()
        M = robot_state[RobotState.M.value]
        ...
        dq = robot_state[RobotState.DQ.value]
        ...
        if self.use_g:
            u_all += self.data.qfrc_bias[self.robot.joint_ids_all]
