...
from shared_control import Device, Robot
...
class MujocoApp():
    def __init__(self, robot_config_file : str = None, scene_file : str = None, use_sim : bool = True):
        main_dir = os.path.dirname(shared_control.__file__)
        ...
        self.model.vis.global_.offheight = self.image_height
        self.model.vis.global_.offwidth = self.image_width
        self.model.vis.quality.shadowsize = 64

        self.data = mujoco.MjData(self.model)
        self.devices = np.array([Device(dev, self.model, self.data, use_sim) for dev in self.config['devices']])
    ...
    def create_robot_devices(self, robot_yml: Dict, use_sim: bool):
        ...
        for rbt in robot_yml:
            ...
            robot = Robot(self.devices[robot_device_idxs], rbt['name'], self.model, self.data, use_sim)
    ...
    def set_free_joint_qpos(self, free_joint_name, quat=None, pos=None):
        jnt_id = self.model.joint_name2id(free_joint_name)
