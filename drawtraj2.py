...
infinity_trajectory(center=(0, 0), size=(1.5, 1.5), num_points=500, plot=True)
plt.show()
# %%
...
def run_ik_recreation(shape_name, x_traj, y_traj):
    ...
    # 2. Train DMP
    traj = np.vstack((x_traj, y_traj))
    dmp = DMPs_rhythmic(n_dmps=2, n_bfs=15, dt=0.001)
...
def perform_ball_counting(model, data):
    """
    Counts balls within the grid segments and prints the distribution.
    Adapted for standalone use with the current model/data.
    """
    # --- Configuration matches your previous class settings ---
    ...
    # 2. Collect Ball Positions
    ball_positions = []
    # Loop through expected ball names
    for i in range(num_balls):
        name = f"ball_{i + 1}"
        body_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, name)

        if body_id != -1:
            # Get current global position of the ball
            pos = data.xpos[body_id][:2]  # x, y only

            # Filter: Only count balls actually inside the workspace bounds
            if (x_min <= pos[0] <= x_max) and (y_min <= pos[1] <= y_max):
                ball_positions.append(pos)

    ball_positions = np.array(ball_positions)

    # 3. Bin Balls into Grid Cells
    if len(ball_positions) > 0:
        for pos in ball_positions:
            x, y = pos
            # searchsorted returns the index where x would fit in the edges array
            i = np.searchsorted(x_edges, x, side='right') - 1
            j = np.searchsorted(y_edges, y, side='right') - 1

            # Safety clamp to ensure indices don't exceed array bounds
            i = min(max(i, 0), num_x_segments - 1)
            j = min(max(j, 0), num_y_segments - 1)

            grid_counts[i, j] += 1

    # 4. Orient for display (Reverse columns to match visual layout if needed)
    grid_counts_visual = grid_counts[:, ::-1]

    # 5. Print Report
    print("\n" + "=" * 40)
    print("       FINAL BALL COUNT DISTRIBUTION       ")
    print("=" * 40)
    for i in range(num_x_segments):
        for j in range(num_y_segments):
            count = grid_counts_visual[i, j]
            # Formatted string for cleaner output
            range_str = f"x:[{x_edges[i]:.2f}, {x_edges[i + 1]:.2f}] y:[{y_edges[j]:.2f}, {y_edges[j + 1]:.2f}]"
            print(f"Grid Cell ({i + 1}, {j + 1}) | {range_str} : {count} balls")

    print("-" * 40)
    print(f"TOTAL BALLS COUNTED: {np.sum(grid_counts)}")
    print("=" * 40 + "\n")
...
if __name__ == "__main__":
    ...
    print(f"Planning trajectory for {dmp.timesteps} steps...")

    for _ in range(dmp.timesteps):
        # A. Get DMP Target
        dmp_pos_2d, _, _ = dmp.step(tau=1)
        target_3d = np.array([dmp_pos_2d[0], dmp_pos_2d[1], MOP_Z_HEIGHT])
    ...
    csv_path = os.path.join(results_dir, "trajectory_output.csv")
    CSV_FIXED_Z = 0.54

    print(f"Executing trajectory and saving to {csv_path}...")
    ...
    with open(csv_path, mode='w', newline='') as csv_file:
        ...
        # Loop through planned trajectory
        for i, joints in enumerate(joint_traj):
            ...
            # Write row to CSV: step, actual_x, actual_y, fixed_z
            writer.writerow([i, cl_pos[0], cl_pos[1], CSV_FIXED_Z])

    print("Discrete trajectory execution complete.")
    perform_ball_counting(model, data)

    # Process arrays for plotting
    dmp_desired_xy = np.array(dmp_desired_xy).T
